...
export function TraceNode({ event, startTime, totalDuration, isLast, onSelect, isSelected }: TraceNodeProps) {
  ...
  return (
    <div className="flex items-center gap-4 py-2">
      {/* 이벤트 타입 라벨 */}
      <div className="w-24 flex-shrink-0 text-sm font-medium">{getEventTypeLabel(event.eventType)}</div>
      ...
      <div className="relative flex-1">
        ...
        {/* 연결선 */}
        {!isLast && <div className="absolute left-0 top-10 h-6 w-px bg-gray-300" />}
      </div>

      {/* 시작 시간 */}
      <div className="w-20 flex-shrink-0 text-right text-xs text-gray-600">+{formatDuration(relativeStart)}</div>
    </div>
  )
}
