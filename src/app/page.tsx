  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <MetricsCard title="총 클릭" value={formatNumber(metrics.totalClicks)} icon={MousePointerClick} />
        <MetricsCard title="전환율" value={formatPercent(metrics.averageConversionRate)} icon={TrendingUp} />
        <MetricsCard title="총 매출" value={formatCurrency(metrics.totalRevenue)} icon={DollarSign} />
        <MetricsCard title="평균 전환 시간" value={formatDuration(metrics.averageConversionTime)} icon={Clock} />
      </div>
